...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    // create the filtering object: downsample the dataset using a leaf size fo .2m
    ...
    pcl::CropBox<PointT> region(true); // you are dealing with the points inside that CropBox
    ...
    roof.filter(indices); // the indices of these points inside cloudRegion that fit inside this box.
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SeparateClouds(pcl::PointIndices::Ptr inliers, typename pcl::PointCloud<PointT>::Ptr cloud) 
{
  // TODO: Create two new point clouds, one cloud with obstacles and other with segmented plane
    ...
    pcl::ExtractIndices<PointT> extract; // to create teh obstacle Point Cloud from the inliers
    typename pcl::PointCloud<PointT>::Ptr planeCloud (new pcl::PointCloud<PointT> ());
    typename pcl::PointCloud<PointT>::Ptr obstacles (new pcl::PointCloud<PointT> ());
    extract.setInputCloud(cloud);
    extract.setIndices(inliers);
    extract.setNegative(false); // if false, get plane cloud
    extract.filter(*planeCloud);

    // create the filtering object
    extract.setNegative(true); // if true, get obstacle cloud
    extract.filter(*obstacles);
    
    /*
    // other methods to get planeCloud:
    for(int index : inliers->indices)
        planeCloud->points.push_back(cloud->points[index]);
    */

    std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> segResult(obstacles, planeCloud);
    return segResult;
}
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
	...
	pcl::PointIndices::Ptr inliers {new pcl::PointIndices}; // represent all the indices of points that belong to the plane(road)
...
std::unordered_set<int> ProcessPointClouds<PointT>::Ransac(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	srand(time(NULL));
	std::cout << "the number of the cloud point" << cloud->points.size() << std::endl;
	...
	// implementing RANSAC for the planes
	while(maxIterations--) {
		// randomly pick two points

		std::unordered_set<int> inliers;
		while (inliers.size() < 3) // in order to avoide picking the same index twice in a row.
		...
		for(int index = 0; index < cloud->points.size(); index++) {
			...
			if(inliers.count(index) > 0) // searches the container for elements with a value of k and returns the number of elemtns found.
				continue;
